<script setup>
  import { getWaitAdoptRecommendAnimalList, searchWaitAdoptAnimal } from '@/apis/animal'
  ...
  const handleGetRecommendAnimalList = async (num) => {
    const res = await getWaitAdoptRecommendAnimalList(num)
    if (res.status === '200') {
  ...
  const handleSearchAnimal = async (keyStr, pageNum, pageSize) => {
    ...
    const res = await searchWaitAdoptAnimal(keyArray, pageNum, pageSize)
    if (res.status === '200') {
      animalDataList.value = res.data.animal_list
